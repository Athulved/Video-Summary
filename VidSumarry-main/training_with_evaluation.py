...
# Evaluation function
def evaluate_model(model, features, annotations, threshold=0.3):
    """
    Evaluate the model using Precision, Recall, F1-Score, and mAP.
    """
    ...
    # Ground truth scores
    gt_scores = annotations["gt_score"].squeeze()

    # Convert predictions to binary values using the threshold
    binary_predictions = (predictions >= threshold).astype(int)

    # Ensure the ground truth scores are binary
    binary_gt_scores = (gt_scores >= threshold).astype(int)

    # Compute Precision, Recall, and F1-Score
    if np.sum(binary_predictions) == 0:
        print("Warning: No positive predictions. Adjust the threshold or improve the model.")
        precision, recall, f1 = 0.0, 0.0, 0.0
    else:
        precision = precision_score(binary_gt_scores, binary_predictions)
        recall = recall_score(binary_gt_scores, binary_predictions)
        f1 = f1_score(binary_gt_scores, binary_predictions)

    # Compute Mean Average Precision (mAP)
    mAP = average_precision_score(binary_gt_scores, predictions)
...
for file_name in os.listdir(features_dir):
    if file_name.endswith(".npy"):
        ...
        # Evaluate the model
        precision, recall, f1, mAP = evaluate_model(model, extracted_features, annotations, threshold=0.3)
        ...
        with torch.no_grad():
            predictions = model(torch.tensor(extracted_features, dtype=torch.float32)).squeeze().numpy()
        print("Predictions:", predictions)
        print("Ground Truth Scores:", gt_score)
